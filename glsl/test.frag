...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
  ...
  vec3 cw = normalize(ta-ro);
	...
	vec3 raydirec = normalize( p.x*cu + p.y*cv + fov*cw );
	...
	vec3 col;//color
	vec4 tra;
    float t = raycast( ro, raydirec, tra, fov, cc );
    if( t<0.0 )
    {
     	col = 1.3*vec3(0.8,.95,1.0)*(0.7+0.3*raydirec.y);
		col += vec3(0.8,0.7,0.5)*pow( clamp(dot(raydirec,light1),0.0,1.0), 32.0 );
	}
	else
	{
		vec3 pos = ro + t*raydirec;
    vec3 nor = calcNormal( pos, t, fov, cc );
    vec3 hal = normalize( light1-raydirec);
    vec3 ref = reflect( raydirec, nor );
    ...
    float fre = 0.04 + 0.96*pow( clamp(1.0-dot(-raydirec,nor),0.0,1.0), 5.0 );
